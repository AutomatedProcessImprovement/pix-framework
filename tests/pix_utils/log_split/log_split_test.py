...
from pix_utils.log_split.log_split import split_log_training_validation_event_wise, split_log_training_validation_trace_wise


def test_split_log_training_validation_event_wise():
    # Create event log mock
    data = pd.DataFrame({
        'case_id': ['0', '0', '0', '1', '0', '1', '2', '0', '1', '2', '1', '2', '3', '1', '2', '3', '2', '3', '3', '3'],
    ...
    train, test = split_log_training_validation_event_wise(data, DEFAULT_CSV_IDS, 0.5, remove_partial_traces_from_validation=True)
    # Assert expected result
    assert train.equals(data.head(10))
