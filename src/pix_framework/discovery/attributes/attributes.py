...
from discovery import discover_fixed_global_attributes
from ge_discrete_attributes import discover_global_and_event_discrete_attributes
from discovery import discover_global_and_event_continuous_attributes
...
def discover_attributes(event_log: pd.DataFrame,
    ...
    g_log, e_log, encoders = preprocess_event_log(event_log, log_ids, sampling_size)

    # GLOBAL FIXED ATTRIBUTES
    global_fixed_attributes = discover_fixed_global_attributes(e_log, all_attribute_columns, confidence_threshold, encoders)
    ...
    # CASE ATTRIBUTES
    case_attributes, case_attribute_metrics = discover_case_attributes(e_log, attributes_to_discover, encoders, log_ids)
    ...
    discrete_results = discover_global_and_event_discrete_attributes(
        e_log, g_log,
        e_log_features, g_log_features,
        attributes_to_discover, encoders, log_ids)
    discrete_results = determine_attribute_type_and_best_model(discrete_results, "KS")
    global_discrete_attributes, event_discrete_attributes = \
        format_attribute_results(discrete_results, encoders[log_ids.activity])
    ...
    continuous_results = discover_global_and_event_continuous_attributes(e_log, g_log,
                                                                         e_log_features, g_log_features,
                                                                         attributes_to_discover, log_ids)
    continuous_results = determine_attribute_type_and_best_model(continuous_results, "EMD")
    global_continuous_attributes, event_continuous_attributes = \
        format_attribute_results(continuous_results, encoders[log_ids.activity])
...
def format_attribute_results(discrete_results, encoder):
    global_attributes = []
    event_attributes_dict = {}

    for attr_name, attr_data in discrete_results.items():
