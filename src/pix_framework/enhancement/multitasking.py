...
_RESOURCE_TAG = "org:resource"
_START_TIMESTAMP_TAG = "start_timestamp"
_END_TIMESTAMP_TAG = "time:timestamp"
...
def adjust_durations(
    ...
    if not is_concurrent:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = (
                executor.submit(_make_aux_log, log, resource) for resource in resources
            )
    ...
    if verbose and metrics_before is not None:
        ...
        print(
            "Utilization before equals the one after: ",
            metrics_before["utilization"] == metrics["utilization"],
        )
        print(
            "Resource events equal:",
...
def _adjust_duration_for_resource(log_interval_df, resource):
    resource_events = log_interval_df[log_interval_df[_RESOURCE_TAG] == resource]
...
def _make_custom_records(resource_events: pd.DataFrame, log: pd.DataFrame):
    ...
    for i, event in resource_events.iterrows():
        start_timestamp = event[_START_TIMESTAMP_TAG]
        end_timestamp = event[_END_TIMESTAMP_TAG]
        resource = event[_RESOURCE_TAG]
...
def _update_end_timestamps(
    records: List[_AuxiliaryLogRecord], log_df: pd.DataFrame
) -> pd.DataFrame:
    """Modifies end timestamp according to the adjusted durations."""
    records = sorted(
        records, key=lambda record: record.event_id
    )  # groupby below works only on sorted data
    for event_id, group in groupby(records, lambda record: record.event_id):
        ...
        log_df.at[event_id, _END_TIMESTAMP_TAG] = log_df.loc[event_id][
            _START_TIMESTAMP_TAG
...
def _resource_metrics(log: pd.DataFrame) -> dict:
    ...
    resources = log[_RESOURCE_TAG].unique()
    utilization = {}
    number_of_events = {}
    for resource in resources:
        events = log[log[_RESOURCE_TAG] == resource]
        number_of_events[resource] = len(events)
        max_end = events[_END_TIMESTAMP_TAG].max()
        min_start = events[_START_TIMESTAMP_TAG].min()
        end_timestamps = events[_END_TIMESTAMP_TAG]
        start_timestamps = events[_START_TIMESTAMP_TAG]
